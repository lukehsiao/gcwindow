...
static char fg_text[] =
    ...
    "men, groping in the Arctic darkness, had found a yellow metal, and "
    "because "
    "steamship and transportation companies were booming the find, thousands "
    "of "
...
static char bg_text[] =
    ...
    "xxx, xxxxxxx xx xxx Xxxxxx xxxxxxxx, xxx xxxxx x xxxxxx xxxxx, xxx "
    "xxxxxxx "
    "xxxxxxxxx xxx xxxxxxxxxxxxxx xxxxxxxxx xxxx xxxxxxx xxx xxxx, xxxxxxxxx "
    "xx "
    "xxx xxxx xxxxxxx xxxx xxx Xxxxxxxxx.  Xxxxx xxx xxxxxx xxxx, xxx xxx xxxx "
    "xxxx xxxxxx xxxx xxxxx xxxx, xxxx xxxxxx xxxxxxx xx xxxxx xx xxxx, xxx "
    "xxxxx "
...
static int create_text_bitmaps(int hwsurface) {
    ...
    get_new_font(
        "Courier New", SCRHEIGHT / 32, 0); /* create a monospaced font */
    ...
    fgbm = SDL_CreateRGBSurface(
        ...
        0,
        0);
    bgbm = SDL_CreateRGBSurface(
        (hwsurface == 2) ? (SDL_HWSURFACE | SDL_ASYNCBLIT) : SDL_SWSURFACE,
        window->w,
        window->h,
        dispinfo.bits,
        0,
    ...
    SDL_FillRect(fgbm,
                 NULL,
                 SDL_MapRGB(fgbm->format,
                            target_background_color.r,
                            target_background_color.b,
                            target_background_color.g));
    SDL_FillRect(bgbm,
                 NULL,
                 SDL_MapRGB(bgbm->format,
                            target_background_color.r,
                            target_background_color.b,
                            target_background_color.g));
...
*/
SDL_Surface *image_file_bitmap(char *fname,
                               int keepsize,
                               int dx,
                               int dy,
                               int hwsurf) {
        ...
        if (zoomed) {
            if (hwsurf) {
                SDL_Surface *hwsurface =
                    SDL_CreateRGBSurface(SDL_HWSURFACE | SDL_ASYNCBLIT,
                                         zoomed->w,
                                         zoomed->h,
                                         dispinfo.bits,
                                         0,
...
static int create_image_bitmaps(int type) {
    // char imagepathsac[1024], imagepathsac_blur[1024];
    // strcpy(imagepathsac, get_resources_path());
    // strcat(imagepathsac, "/sacrmeto.jpg");

    // strcpy(imagepathsac_blur, get_resources_path());
    // strcat(imagepathsac_blur, "/sac_blur.jpg");

    clear_full_screen_window(target_background_color);
    get_new_font(
        "Courier New", SCRHEIGHT / 32, 1); /* create a monospaced font */
    ...
    switch (type) {
        ...
        {
            fgbm = image_file_bitmap(
                "images/sacrmeto.jpg", 0, SCRWIDTH, SCRHEIGHT, 1);
        ...
        case 2: /* normal and blurred bitmaps, stretched to fit display */
            fgbm = image_file_bitmap(
                "images/sacrmeto.jpg", 0, SCRWIDTH, SCRHEIGHT, 1);
            bgbm = image_file_bitmap(
                "images/sac_blur.jpg", 0, SCRWIDTH, SCRHEIGHT, 1);
            //		fgbm = image_file_bitmap(imagepathsac, 0,
            // SCRWIDTH,SCRHEIGHT,1); 		bgbm =
            // image_file_bitmap(imagepathsac_blur, 0, SCRWIDTH,SCRHEIGHT,1);
            break;
    }
...
int do_gcwindow_trial(int num) {
    int i;
    char imgtextpath[1024], path[1024], imgsacpath[1024], imgsacblurpath[1024],
        savetextpath[1024];
    ;

#if defined(MACOSX) && defined(MACAPP)
    strcpy(imgtextpath, "!V IMGLOAD FILL ");
    strcat(imgtextpath, get_resources_path());
    strcat(imgtextpath, "/images/text.png");

    strcpy(path, get_resources_path());
    strcat(path, "/images/text.png");

    strcpy(imgsacpath, "!V IMGLOAD FILL ");
    strcat(imgsacpath, get_resources_path());
    strcat(imgsacpath, "/images/sacrmeto.jpg");

    strcpy(imgsacblurpath, "!V IMGLOAD FILL ");
    strcat(imgsacblurpath, get_resources_path());
    strcat(imgsacblurpath, "/images/sac_blur.jpg");

    strcpy(savetextpath, "!V IMGLOAD FILL ");
    strcat(savetextpath, get_resources_path());
    strcat(savetextpath, "/images");
#else
    strncpy(path, "images", 1024);
    strncpy(imgsacpath, "!V IMGLOAD FILL images/sacrmeto.jpg", 1024);
    strncpy(imgtextpath, "!V IMGLOAD FILL images/text.png", 1024);
    strncpy(imgsacblurpath, "!V IMGLOAD FILL images/sacrmeto.jpg", 1024);
    ...
    switch (
        num) { /* #1: gaze-contingent text, normal in window, "xx" outside */
        case 1:
            ...
            */
            ...
            /* IMGLOAD command is recorded for EyeLink Data Viewer analysis
               It displays a default image on the overlay mode of the trial
               viewer screen. Writes the image filename + path info
            */
            ...
            //
            eyemsg_printf(imgtextpath);
            bitmap_save_and_backdrop(fgbm,
                                     0,
                                     0,
                                     0,
                                     0,
            ...
            i = gc_window_trial(
                ...
                SCRHEIGHT / 3,
                0,
                60000L); /* Gaze-contingent window, normal text */
            SDL_FreeSurface(fgbm);
            fgbm = NULL;
            SDL_FreeSurface(bgbm);
            bgbm = NULL;
        ...
        case 2: /* #2: gaze-contingent text, "xx" in window, normal outside */
            ...
            i = gc_window_trial(
                ...
                SCRHEIGHT / 3,
                1,
                60000L); /* Gaze-contingent window, masked text */
            SDL_FreeSurface(fgbm);
            fgbm = NULL;
            SDL_FreeSurface(bgbm);
            bgbm = NULL;
        ...
        case 3: /* #3: Image, normal in window, blank outside */
            ...
            bitmap_to_backdrop(
                ...
                0,
                0,
                (UINT16)(BX_MAXCONTRAST |
                         ((eyelink_get_tracker_version(NULL) >= 2)
                              ? 0
                              : BX_GRAYSCALE)));

            eyemsg_printf(imgsacpath);
            ...
            i = gc_window_trial(
                ...
                SCRHEIGHT / 3,
                0,
                60000L); /* Gaze-contingent window, normal image */
            SDL_FreeSurface(fgbm);
            fgbm = NULL;
            SDL_FreeSurface(bgbm);
            bgbm = NULL;
        ...
        case 4: /* #4: Image, blank in window, normal outside */
            ...
            i = gc_window_trial(
                ...
                SCRHEIGHT / 3,
                1,
                60000L); /* Gaze-contingent window, masked image */
            SDL_FreeSurface(fgbm);
            fgbm = NULL;
            SDL_FreeSurface(bgbm);
            bgbm = NULL;
        ...
        case 5: /* #5: Image, blurred outside window */
            ...
            // eyemsg_printf("!V IMGLOAD FILL images/sacrmeto.jpg");   //should
            // be eyemsg_printf("!V IMGLOAD FILL images/sac_blur.jpg");
            eyemsg_printf(imgsacblurpath);
    ...
    */
    ...
    for (trial = 1; trial <= NTRIALS; trial++) {
        if (eyelink_is_connected() == 0 ||
            break_pressed()) /* drop out if link closed */
        {
            return ABORT_EXPT;
        }
        /* RUN THE TRIAL */
